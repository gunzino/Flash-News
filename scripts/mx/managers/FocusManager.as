package mx.managers
{
   ...
   public class FocusManager implements IFocusManager
   {
      ...
      {
         var _loc2_:DisplayObject = DisplayObject(form);
      ...
      private function tabEnabledChangeHandler(param1:Event) : void
      {
         ...
         var _loc3_:int = focusableObjects.length;
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
      ...
      {
         var _loc2_:ISystemManager = form.systemManager;
      ...
      private function getNextFocusManagerComponent2(param1:Boolean = false, param2:DisplayObject = null, param3:int = -2) : FocusInfo
      {
         ...
         if(param3 == FROM_INDEX_UNSPECIFIED)
         {
            ...
            if(!_loc10_)
            {
               _loc10_ = form.systemManager.stage.focus;
            }
         ...
         _loc9_.displayObject = findFocusManagerComponent2(focusableCandidates[_loc7_]);
      ...
      private function removeFromParentBridge(param1:Event) : void
      {
         ...
         if(_loc2_.useSWFBridge())
         {
            ...
            if(_loc3_ && !(form.systemManager is SystemManagerProxy))
            {
      ...
      private function setFocusToComponent(param1:Object, param2:Boolean) : void
      {
         ...
         if(param1)
         {
            if(param1 is ISWFLoader && ISWFLoader(param1).swfBridge)
            {
               ...
               _loc4_ = ISWFLoader(param1).swfBridge;
               if(_loc4_)
               {
      ...
      private function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         {
            if(!_lastFocus && _loc3_ is IEventDispatcher && SystemManager(form.systemManager).useSWFBridge())
            {
      ...
      private function focusInHandler(param1:FocusEvent) : void
      {
         ...
         var _loc2_:InteractiveObject = InteractiveObject(param1.target);
         var _loc3_:ISystemManager = form.systemManager;
      ...
      {
         var _loc1_:ISystemManager = form.systemManager;
         if(_loc1_)
         {
      ...
      private function getIndexOfNextObject(param1:int, param2:Boolean, param3:Boolean, param4:String) : int
      {
         ...
         while(true)
         {
            ...
            {
               _loc7_ = DisplayObject(findFocusManagerComponent2(focusableCandidates[param1]));
      ...
      public function moveFocus(param1:String, param2:DisplayObject = null) : void
      {
         ...
         var _loc4_:FocusEvent = new FocusEvent(FocusEvent.KEY_FOCUS_CHANGE);
         _loc4_.keyCode = Keyboard.TAB;
      ...
      private function getMaxTabIndex() : int
      {
         ...
         var _loc1_:Number = 0;
         var _loc2_:int = focusableObjects.length;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
            _loc4_ = focusableObjects[_loc3_].tabIndex;
            if(!isNaN(_loc4_))
            {
      ...
      public function removeSWFBridge(param1:IEventDispatcher) : void
      {
         ...
         if(_loc3_)
         {
            _loc4_ = focusableObjects.indexOf(_loc3_);
            if(_loc4_ != -1)
            {
               focusableObjects.splice(_loc4_,1);
               calculateCandidates = true;
      ...
      {
         var _loc3_:int = 0;
         var _loc2_:InteractiveObject = form.systemManager.stage.focus;
         if(!_loc2_)
         {
            _loc3_ = !!param1?0:int(focusableCandidates.length - 1);
      ...
      private function removedHandler(param1:Event) : void
      {
         ...
         var _loc4_:DisplayObject = !!focusPane?focusPane.parent:null;
         if(_loc4_ && _loc3_ != focusPane)
         {
            if(_loc3_ is DisplayObjectContainer && isParent(DisplayObjectContainer(_loc3_),focusPane))
            {
      ...
      private function dispatchEventFromSWFBridges(param1:Event, param2:IEventDispatcher = null) : void
      {
         ...
         var _loc4_:ISystemManager = form.systemManager;
         if(!popup)
         {
            _loc7_ = swfBridgeGroup.parentBridge;
            if(_loc7_ && _loc7_ != param2)
            {
               _loc3_ = param1.clone();
      ...
      public function set showFocusIndicator(param1:Boolean) : void
      {
         ...
         if(_loc2_ && !popup && form.systemManager.swfBridgeGroup)
         {
